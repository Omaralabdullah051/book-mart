...
const ViewProfile = () => {
  ...
  useEffect(() => {
    if (!user?.photoURL) {
      (async () => {
        await updateProfile({
          photoURL: "https://i.postimg.cc/mk6QDsQm/5945893.png",
        });
      })();
    }
  }, [user, updateProfile]);
  ...
  return (
    <div className="bg-gray-800 m-5 md:m-10 rounded-2xl p-4 md:p-10">
      ...
      <div>
        <img
          className="w-40 mx-auto md:mx-0 rounded-full md:ml-20 xl:ml-32"
          src={user?.photoURL}
          alt=""
        />
      </div>
      <div className="text-green-600 mt-4 font-bold md:pl-20 xl:pl-32 md:mt-2 xl:pr-32">
        ...
        <button
          ...
          className="px-3 py-1 md:px-8 md:py-2 text-xs md:text-base bg-green-600 text-gray-300 rounded font-bold mt-2  hover:bg-green-400 hover:text-black focus:ring-4 focus:ring-offset-slate-800"
        >
          Info
        </button>
        <button
          onClick={() => signOut(auth)}
          className="px-3 py-1 md:px-8 md:py-2 text-xs md:text-base bg-green-600 text-gray-300 rounded font-bold mt-2  hover:bg-green-400 hover:text-black ml-4 md:ml-8 focus:ring-4 focus:ring-offset-slate-800"
        >
          Sign Out
        </button>
      </div>
